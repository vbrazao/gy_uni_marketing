...
```{r}
#| label: fig-timeline
#| fig-cap: Timeline of posts by university 
#| fig-width: 7.5
#| fig-height: 7
...
dat |> 
  ...
  ggplot(aes(x = date_created, y = category, color = category)) +
  ggbeeswarm::geom_beeswarm(alpha = 0.8, cex = 1.1, priority = "descending") +
...
dat |> 
  mutate(
    date_created = lubridate::as_datetime(date_created),
    week = lubridate::isoweek(date_created)
  ) |> 
  ggplot(aes(x = factor(week), y = category, color = category)) +
  geom_count() +
  scale_color_viridis_d(option = "A", begin = 0.2, end = 0.8) +
  coord_cartesian(xlim = c(timeline_start,
                           timeline_end)) +
...
dat |> 
  mutate(
    date_summary = lubridate::as_datetime(date_created) |> lubridate::ceiling_date(unit = "week", week_start = 5),
  ) |> 
  ggplot(aes(x = date_created, y = category, color = category)) +
  geom_count(aes(x = date_summary)) + theme(legend.position = "none") +
  scale_color_viridis_d(option = "A", begin = 0.2, end = 0.8) +
  coord_cartesian(xlim = c(timeline_start, timeline_end)) +
  labs(x = "", y = "")
...
tab_weeks <- dat |> 
  mutate(
    date_summary = lubridate::as_datetime(date_created) |> lubridate::ceiling_date(unit = "week", week_start = 5)
      ...
      count("Week ending on" = date_summary, category, .drop = FALSE) |> 
  tidyr::pivot_wider(
    names_from = category,
    values_from = n, 
  ...
  mutate(
    "Week ending on" = lubridate::as_date(`Week ending on`)
  )
...
marginaleffects::avg_predictions(
  ...
  geom_pointrange() +
  labs(x = "", y = "Total Interactions", caption = stringr::str_wrap("Note: Points represent expected values for the mean number of total interactions, lines show corresponding 95% Confidence Intervals", 80)) +
  coord_flip()
```

We compute pairwise differences to evaluate which universities receive significantly more (or fewer) interactions than the others. For brevity, @tbl-total-uni-pairs below displays all the significant estimates for pairwise differences. For the unabridged table and accompanying plots, see XXX.
...
marginaleffects::avg_predictions(
  ...
  ggplot(aes(x = category, y = estimate, ymin = conf.low, ymax = conf.high)) +
  geom_pointrange() +
  labs(x = "", y = "Positive Interactions", caption = stringr::str_wrap("Note: Points represent expected values for the mean number of positive interactions, lines show corresponding 95% Confidence Intervals", 80)) +
  coord_flip()
```
